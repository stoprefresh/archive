...
public class StateControllerTests {
  ...
  public void test_GET_GetStateDO_with_name_param_sets_view_WEBINF_resultJSP() {
    try {
     ...
     assertThat(modelAndView, allOf(hasProperty(""))
  ...
  public void test_GET_GetStateDO_with_abbr_param_sets_view_WEBINF_resultJSP() {
	  ...
	  try {
		  MvcResult result = mockMvc.perform(get("/GetStateData.do")
				  .param("abbr", "MI"))
				  .andExpect(status().isOk()).andReturn();
		  ModelAndView modelAndView = result.getModelAndView();
		  ModelMap map = modelAndView.getModelMap();
  ...
  public void test_POST_sets_WEBINFresultJSP_as_view_and_adds_State_to_dao() {
    try {
      MvcResult result = mockMvc.perform(post("/NewState.do/").param("capital", "Here").param("abbrevaiation", "RL"))
      		+ .param("name", "Robland").andExpect(status().isOk()).andReturn();
